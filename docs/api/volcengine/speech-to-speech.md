...
> 本文档包含端到端实时语音大模型的产品简介和API接入文档，从火山引擎文档站点抓取。

**来源：** https://www.volcengine.com/docs/6561/1594356
...
|**超自然对话效果** |1. 更接近真人在语音聊天中的对话口语习惯和语音表现力
...
固定值：volc.speech.dialog |是 |volc.speech.dialog |
...
|100 |StartSession |Session类事件 |asr字段用于描述识别阶段的相关配置：
...
* tts_2.0_model字段用于传递复刻音色效果，高表现力版本复刻音色固定传 `expressive`，当前仅支持O2.0版本模型 |```JSON```
```{```
``` "tts": {```
``` "extra": {```
...
|201 |UpdateConfig ||客户端更新通话过程中的SP相关配置，其中dialog_id字段代表上下文唯一标识

**注意**：右侧涉及配置字段均采用全量覆盖模式，请确保请求中包含完整字段信息 |```JSON```
```{```
``` "tts": {```
``` "speaker": {{STRING}},```
``` "audio_config": {```
...
```}```
 |
|400 |EndASR ||客户端在麦克风按键输入模式下即**push_to_talk**模式，需要在音频输入结束时向服务端发送音频结束信号 |```JSON```
```{}```
 |
...
```}```
 |
|510 |ConversationCreate |上下文管理事件 |上下文追加规则：
...
* 时间戳策略需保持一致：要么所有记录均携带时间戳，要么全部不携带，不能混用 |```JSON```
```{```
``` "items":[```
``` {```
``` "role": {{STRING}},```
...
* 同理，若传入助手侧 item_id，系统也会删除与其对应的用户消息，确保上下文不出现不完整对话 |```JSON```
```{```
``` "items":[```
``` {```
``` "item_id": {{STRING}}```
...
```}```
 |
|515 |ClientInterrupt | |在麦克风按键输入模式下即**push_to_talk**模式，用于客户端打断服务端响应，便于进行下一次识别处理 |```JSON```
```{}```
 |
